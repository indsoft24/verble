...
const UPSC_SLUG = 'upsc-civil-services'; 

/**
 * @desc    Get featured UPSC courses
 * @route   GET /api/ias/featured-courses
 * @access  Public
 */
export const getFeaturedUpscCourses = asyncHandler(async (req, res) => {
    const upscCategory = await ExamCategory.findOne({ slug: UPSC_SLUG });
    if (!upscCategory) {
        return res.status(200).json({ status: 'success', data: { courses: [] } });
    }

    const courses = await Course.find({ isPublished: true, examCategory: upscCategory._id })
        .limit(6)
...
/**
 * @desc    Get all published UPSC courses (paginated)
 * @route   GET /api/ias/courses
 * @access  Public
 */
export const getUpscCourses = asyncHandler(async (req, res) => {
    ...
    const queryConditions = { isPublished: true, examCategory: upscCategory._id };
...
/**
 * @desc    Get all published UPSC videos (paginated)
 * @route   GET /api/ias/videos
 * @access  Public
 */
export const getUpscVideos = asyncHandler(async (req, res) => {
    const page = parseInt(req.query.page) || 1;
    ...
    const coursesInCategory = await Course.find({ examCategory: upscCategory._id }).select('_id');
